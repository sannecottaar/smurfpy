        ...
        if hasattr(seis[0],filt):
             print('done with', stalist[i])
                ...
                if flag=='SH':
                    ...
                    SVref=transverse.slice(Ptime-25.,Ptime+150.) # Cut out P arrival on transverse, still named SVref for simplicitiy

                # Resample data to 10 samples/sec (instead of 40 samples/sec). This greatly speeds up all the computations
                SVref.resample(10.)
                ...
                #check that both components are not zero length otherwise script will crash
