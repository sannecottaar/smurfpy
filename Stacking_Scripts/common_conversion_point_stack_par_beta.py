...
#############################################################################
    ...
    def start_empty_volume_master(self,name='Megavolume',filter='jgf1',conversion='ak135',  factor=2., lonmin=None, lonmax=None, lonrez=None, latmin=None,latmax=None,latrez=None, depmin=None,depmax=None,deprez=None):
        ...
        filter  Sring that refers to the name of the receiver function to be used (If you add a new one, this needs to be implemented in the code)
        conversion String for the  conversion model to be used (default = ak135)
        ...
        outfilename='../CCP_volumes/'+name+'_'+filter+'_'+conversion+'_'+str(factor)+'/Stack_master.PICKLE'
        ...
        weightedvolumesquares= np.zeros([len(grid_lon),len(grid_lat),len(grid_depth)]) # USed for weighted differences in parallel 
        ...
        self.VOL['weightedvolumesquares']=weightedvolumesquares
        # self.VOL['num']=num
        self.VOL['count']=0
...
#############################################################################
# Start a new sub volume dictionary
#############################################################################

    def start_empty_sub_volume(self,name='Megavolume',filter='jgf1',conversion='ak135',  factor=2., sta_num=0, file_num=0, lonmin=None, lonmax=None, lonrez=None, latmin=None,latmax=None,latrez=None, depmin=None,depmax=None,deprez=None):

        #Make directory for the volumes
        outfilename='../CCP_volumes/'+name+'_'+filter+'_'+conversion+'_'+str(factor)+'/Stack_'+str(sta_num)+'_'+str(file_num)+'.PICKLE'
...
#############################################################################
    def load_latest_master(self,name='Megavolume',filter='jgf1',conversion='ak135',factor=2.):
        '''
        Loads latest master-volume
        '''
        # print(name)
        rffilter=filter
...
# Load latest volume to dictionary
#############################################################################
    def load_latest_sub(self,name='Megavolume',filter='jgf1',conversion='ak135',factor=2., sta_num=0, file_num=0):
        '''
        Loads latest sub-volume
        '''
        # print(name)
        rffilter=filter
...
#############################################################################
# Add list of receiver functions to sub Volume
#############################################################################

    def addlist_sub(self,rffile, name='Megavolume',filter='jgf1',conversion='ak135',factor=2., sta_num=0, file_num=0):
        ''' 
